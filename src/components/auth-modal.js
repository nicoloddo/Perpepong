/**
 * Auth Modal Web Component
 * Displays a modal for user login and signup with email/password and Google
 * 
 * Usage:
 * <auth-modal></auth-modal>
 * 
 * To open: window.dispatchEvent(new CustomEvent('open-auth-modal'))
 * To close: window.dispatchEvent(new CustomEvent('close-auth-modal'))
 */
    ...
    this.innerHTML = `
      <div 
        ...
        class="fixed inset-0 z-50 flex items-center justify-center p-4 transition-all ${
          this.isOpen ? 'bg-black bg-opacity-50' : 'hidden'
        }"
      >
        <div class="bg-card rounded-2xl shadow-2xl max-w-md w-full p-6 relative">
          <!-- Close button -->
          <button 
            id="auth-close-btn"
            class="absolute top-4 right-4 text-foreground-light hover:text-foreground transition"
            aria-label="Close"
          >
